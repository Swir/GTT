...
$begin = @($lines | Where-Object { $_ -match 'FARM_CARGO_BREAKDOWN_RUNTIME_BEGIN\s+version=1\s+route=feed-breakdown-tow-hill-wood' }) | Select-Object -Last 1
$prepare = Find-Phase 'PREPARE'
$accept = Find-Phase 'ACCEPT'
$pickup = Find-Phase 'PICKUP'
$request = Find-Phase 'BREAKDOWN_TOW_REQUEST'
...
$complete = @($lines | Where-Object { $_ -match 'FARM_CARGO_BREAKDOWN_RUNTIME_COMPLETE\s+result=PASS\s+route=feed-breakdown-tow-hill-wood' }) | Select-Object -Last 1
$diagnostics = @($lines | Where-Object { $_ -match 'FARM_CARGO_BREAKDOWN_RUNTIME\s+phase=DIAGNOSTIC\s+result=FAIL' })
$preCheckpoint = @($lines | Where-Object { $_ -match 'FARM_CARGO_BREAKDOWN_RECOVERY\s+event=TOW_CHECKPOINT\s+vehicle=.*\s+saved=YES\s+timer_paused=NO\s+transfer_allowed=NO' }) | Select-Object -Last 1
$postRecovery = @($lines | Where-Object { $_ -match 'FARM_CARGO_BREAKDOWN_RECOVERY\s+event=POST_RECOVERY_VERIFY\s+result=PASS\s+vehicle=.*\s+.*identity_preserved=YES\s+saved=YES\s+timer_paused=NO' }) | Select-Object -Last 1
...
$required = [ordered]@{
    PREPARE=$prepare; ACCEPT=$accept; PICKUP=$pickup; BREAKDOWN_TOW_REQUEST=$request; TOW_COMPLETE=$tow;
    WRONG_VEHICLE_AFTER_TOW=$wrong; HILL_HANDOFF=$hill; FINAL_HANDOFF=$final; PERSISTENCE=$persistence
}
if (-not $begin) { $failures.Add('Farm Cargo breakdown runtime begin marker is missing') }
...
if (-not $complete) { $failures.Add('Farm Cargo breakdown route did not complete with PASS') }
if ($diagnostics.Count -gt 0) { $failures.Add("scenario logged $($diagnostics.Count) diagnostic failure(s)") }
if (-not $preCheckpoint) { $failures.Add('production cargo recovery did not prove the pre-tow primary-save checkpoint') }
if (-not $postRecovery) { $failures.Add('production cargo recovery did not prove exact-ID post-tow recovery') }
...
$breakdown = Require-IntegerField $request 'breakdown' 'BREAKDOWN_TOW_REQUEST'
$towRequested = Require-IntegerField $request 'tow_requested' 'BREAKDOWN_TOW_REQUEST'
$severity = Require-NumberField $request 'severity' 'BREAKDOWN_TOW_REQUEST'
$tireAfterDamage = Require-NumberField $request 'tire_integrity' 'BREAKDOWN_TOW_REQUEST'
$timerBefore = Require-NumberField $request 'timer_before' 'BREAKDOWN_TOW_REQUEST'
$integrityBefore = Require-NumberField $request 'integrity_before' 'BREAKDOWN_TOW_REQUEST'
...
if ($null -ne $tireAfterDamage -and $tireAfterDamage -gt 0.08) { $failures.Add("damage probe did not disable tires enough for native roadside recovery (tire=$tireAfterDamage)") }
...
if ($null -ne $payoutDelta -and $payoutDelta -le 0) { $failures.Add("delivery payout did not increase cash after tow (delta=$payoutDelta)") }
...
$completeFields = @('accepted','pickup','breakdown','tow_requested','tow_complete','identity_preserved','timer_continued','integrity_not_improved','damage_preserved','wrong_vehicle_rejected','hill','final','save','authority_cleared')
$completeValues = @{}
...
$evidence = [ordered]@{
    schema = 'gtt.farm-cargo-breakdown-runtime.v1'; result = $result; game = 'Grand Theft Tractor'
    version = $build.version; platform = $build.platform; git_sha = $build.git_sha
    route = 'Feed Depot -> native Mulebox breakdown -> paid roadside tow -> Hill Farm -> North Wood Yard'
    stable_vehicle_id = $completeVehicle; native_breakdown_proven = ($breakdown -eq 1); player_authorized_tow = ($towRequested -eq 1)
    ...
    production_pre_tow_checkpoint = [bool]$preCheckpoint; production_post_tow_identity_verification = [bool]$postRecovery
...
if ($result -ne 'PASS') { Write-Error "Farm Cargo breakdown packaged runtime exercise failed: $($failures -join '; ')"; exit 9 }
Write-Host "[GTT] Farm Cargo breakdown/tow runtime: PASS (vehicle=$completeVehicle, tow=$towCost, payout=+$payoutDelta, rep=+$reputationDelta)."
