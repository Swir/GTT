...
if failed:
    raise SystemExit('Packaged gameplay smoke verification failed: ' + ', '.join(failed))
label = '.'.join(str(part) for part in candidate_version) if candidate_version else 'unknown'
print(f'Packaged gameplay smoke verification passed ({len(checks)} checks) under additive candidate {label}; runtime={minimum_alive}s, gameplay={minimum_gameplay}s, timeout={launch_timeout}s.')
