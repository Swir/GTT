...
for token in [
    ...
    "trailer_dual_contact_samples",
    "trailer_safe_hitch_samples",
]:
...
for token in [
    ...
    "███████████████████░ 96.2%",
    "| **125** | **5** | **130** | **96.2%** |",
    "0.1.18 runtime evidence closure",
]:
    if token not in roadmap:
        errors.append(f"roadmap style/progress/status drift: {token}")

for token in [
    "drivetrain manifest is now mandatory",
    "authored trailer runtime",
    "NATIVE_TRAILER_RUNTIME.json",
    "schema 6",
    "does not close",
]:
...
print("GTT 0.1.18 runtime acceptance closure verification OK")
print(" - technical gate now consumes the deterministic drivetrain evidence")
print(" - authored trailer rig/contact/hitch evidence is mandatory")
