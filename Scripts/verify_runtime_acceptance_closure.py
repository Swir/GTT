...
for token in [
    ...
    "NATIVE_CHAOS_RUNTIME.json", "NATIVE_DRIVETRAIN_SCENARIO.json", "authored_active_samples", "native_tow_samples",
    "dual_contact_samples", "safe_hitch_samples", "hitch_error_cm", "warning", "ExpectedGitSha", "exit 5",
]:
...
for token in ["run_win64_attested_candidate_acceptance.ps1", "WIN64_CANDIDATE_ATTESTATION.json", "FINAL_SHA256SUMS.txt"]:
    if token not in workflow:
        errors.append(f"Win64 evidence workflow missing sealed delegate token: {token}")
for token in ["evaluate_drivetrain_scenario.ps1", "evaluate_authored_trailer_runtime.ps1", "evaluate_demo_candidate.ps1"]:
    if token not in base_runner:
        errors.append(f"base exact-candidate runner missing closure token: {token}")
for token in ["NATIVE_TRAILER_RUNTIME.json", "NATIVE_CHAOS_RUNTIME.json", "FINAL_SHA256SUMS.txt"]:
    if token not in attestor:
        errors.append(f"candidate attestor no longer seals closure evidence: {token}")
...
for token in [
    ...
    "## 📊 Overall progress", "ROADMAP-96.2%25", "DONE-125%2F130", "../assets/readme/progress-mini.svg",
    "| **125** | **5** | **130** | **96.2%** |", "0.1.18 runtime evidence closure",
]:
...
print(" - canonical runtime ordering and sealed evidence are verified behind the workflow delegate")
