...
def main() -> None:
    header = require(
        ...
        "UGTTFarmCargoEvidenceScenarioSubsystem", "UTickableWorldSubsystem", "WrongVehicleProbe",
        "VerifyPersistence", "BaselineLogisticsSave",
    )
    cpp = require(
        ...
        "StartDelaySeconds = 180.0f", "GlobalDeadlineSeconds = 196.0f",
        ...
        "Authority->GetBoundCargoVehicle()", "wrong-vehicle-handoff-was-not-rejected",
        ...
        "GameMode->SaveProgress()", "Logistics->RestoreFromSave(BaselineLogisticsSave)",
    ...
    authority = require(
        ...
        "FARM_CARGO_AUTHORITY event=BIND result=PASS", "FARM_CARGO_AUTHORITY event=HANDOFF_CHECK result=PASS",
        "BoundCargoVehicleId",
    )
    terminal = require(
        "Source/GTT/Private/Activities/GTTFarmJobTerminal.cpp", "ValidateBoundCargoHandoff",
        "Authority->BindLoadedVehicle", "Director->TryCompleteJob(Pawn)", "Director->TryCompleteFinalStop(Pawn)",
    )
    director = require(
        "Source/GTT/Private/Activities/GTTFarmJobDirector.cpp", "Economy->AddCash(TotalReward",
        "Logistics->RecordCargoSuccess", "GameMode->SaveProgress()",
    )
    _ = header, authority, terminal, director

    evaluator = require(
        "Scripts/evaluate_farm_cargo_runtime.ps1", "gtt.farm-cargo-runtime.v1", "-GTTFarmCargoRuntimeScenario",
        "FARM_CARGO_RUNTIME_BEGIN", "wrong-vehicle handoff rejection was not proven", "FARM_CARGO_RUNTIME_COMPLETE",
        "FARM_CARGO_RUNTIME.json", "Require-IntegerField", "missing required integer field",
        "completion marker gate", "evidence disagrees between FINAL_HANDOFF and COMPLETE",
    )
    for required_field in (
        "active_order_tier", "same_vehicle", "payout_delta", "cargo_runs_delta", "reputation_delta",
        "authority_cleared", "accepted", "pickup", "wrong_vehicle_rejected", "hill", "final", "save",
    ):
    ...
    smoke = require(
        "Scripts/smoke_test_windows.ps1", "-GTTDemoSmokeScenario", "-GTTFarmCargoRuntimeScenario",
    ...
    if minimum_alive < 200 or launch_timeout <= minimum_alive or launch_timeout < 220:
        raise AssertionError(f"0.1.29 Farm Cargo runtime window regressed: alive={minimum_alive}, timeout={launch_timeout}")
    ...
    changelog = require(
        "CHANGELOG.d/0.1.29.md", "GTT 0.1.29", "Deterministic Packaged Farm Cargo Runtime Exercise", "does not claim",
    )
    ...
    for token in (
        ...
        "<!-- ROADMAP-PROGRESS:END -->", "## 📊 Overall progress",
        "DONE-125%2F130", "ROADMAP-96.2%25", "| **125** | **5** | **130** | **96.2%** |",
    ...
    readme = require(
        "README.md", "<!-- SWIR-README-STANDARD:v2 -->", "## 🔎 Search Keywords",
        "assets/readme/progress-card.svg", "0.1.29", "FARM_CARGO_RUNTIME.json",
