...
def verify_priority_model() -> None:
    ...
    cargo_reward_match = re.search(r"GetCargoPriorityRewardMultiplier\(\) const\s*\{(?P<body>.*?)\n\}", cpp, re.S)
...
def verify_road_rush_integration() -> None:
    ...
    require(road, "Logistics->GetRoadCourierRewardMultiplier() * Logistics->GetRoadPriorityRewardMultiplier()", "ROAD contract reward lock")
    ...
    for token in (
        "PartsPickupLocation", "HillFarmRelayLocation", "DeliveryLocation", "ParcelIntegrity",
        "NativeImpactCountDuringRun", "bPoliceIncidentDuringRun", "AssessJobReadiness(RoadRunJob)",
        "COURIER HANDOFF BLOCKED", "RecordCourierSuccess", "RecordCourierFailure",
    ):
...
def verify_dispatcher_and_queue_integration() -> None:
    ...
    for token in (
        ...
        "GetPriorityDispatchLabel()", "GetPriorityDispatchSummary()", "GetPriorityVehicleLabel()",
        "GetCargoPriorityUrgency()", "GetRoadPriorityUrgency()",
    ):
        require(dispatcher, token, "dispatcher priority integration")
    for token in (
        "ReserveNegotiatedCargoOrder", "CargoReservedOrderTiers", "CargoReservedUnits",
        "CargoReservationExpiryHours", "GetReservedCargoOrderTier",
    ):
...
def verify_roadmap_style_lock() -> None:
    ...
    for token in (
        ...
        "<!-- ROADMAP-PROGRESS:END -->", "alt=\"CI\"", "ROADMAP-", "DONE-", "STATUS-",
    ...
    # Current presentation standard is SVG-only; numeric truth remains in the canonical table.
    assert roadmap.count("../assets/readme/progress-mini.svg") == 1, "roadmap must embed exactly one progress-mini.svg"
    assert not re.search(r"^[\s>*`-]*[█▓▒░▰▱■□▪▫▮▯]{5,}", roadmap, re.M), "legacy text progress meter must not return"

    assert (completed, remaining, total) == (125, 5, 130), (
        f"0.1.10 unexpectedly changed roadmap completion to {completed}/{total}; only real accepted blockers may alter checklist state"
    )
