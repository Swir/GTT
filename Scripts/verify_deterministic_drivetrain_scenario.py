...
for token in [
    "UGTTDrivetrainEvidenceScenarioSubsystem", "ForwardAcceleration", "BrakeForReverse", "ReverseAcceleration",
    "BrakeForForward", "ForwardReturn", "bAutomaticUpshiftObserved", "bSafeReverseCommitObserved",
...
for token in [
    "StartDelaySeconds = 76.0f", "GlobalDeadlineSeconds = 122.0f", "ShiftReleaseSpeedKmh = 3.5f",
    "ForwardEvidenceSpeedKmh = 6.0f", "ReverseEvidenceSpeedKmh = 5.0f", "NATIVE_DRIVETRAIN_SCENARIO_BEGIN",
    "phase=AUTOMATIC_UPSHIFT", "phase=REVERSE_INTERLOCK", "phase=REVERSE_COMMIT", "phase=REVERSE_MOTION",
    "phase=FORWARD_COMMIT", "phase=FORWARD_MOTION", "NATIVE_DRIVETRAIN_SCENARIO_COMPLETE",
...
for token in [
    "gtt.native-drivetrain-scenario.v1", "NATIVE_CHAOS_RUNTIME.json", "automatic forward upshift was not proven",
    "reverse was committed above the safe release window", "forward return was committed above the safe release window",
    "reverse_motion_signed_speed_kmh", "max_forward_gear_observed", "diagnostic_failure_count", "3.75",
]:
    if token not in evaluator:
        errors.append(f"drivetrain runtime evaluator missing: {token}")

for token in ["evaluate_drivetrain_scenario.ps1", "NATIVE_DRIVETRAIN_SCENARIO.json"]:
...
for token in [
    "python Scripts/verify_deterministic_drivetrain_scenario.py", "python Scripts/verify_automatic_drivetrain_runtime.py",
    "python Scripts/verify_native_runtime_telemetry.py", "python Scripts/verify_deterministic_demo_scenario.py",
]:
...
for token in [
    ...
    "📊 Overall progress", "../assets/readme/progress-mini.svg", "| **125** | **5** | **130** | **96.2%** |",
]:
    if token not in roadmap:
        errors.append(f"SWIR roadmap structure/progress missing: {token}")
...
for token in [
    "Automatic forward shift", "Reverse interlock", "Safe reverse commit", "Measured reverse motion",
    "Safe forward return", "NATIVE_DRIVETRAIN_SCENARIO.json", "visual acceptance",
]:
    if token.lower() not in playtest.lower():
        errors.append(f"0.1.17 playtest missing: {token}")
for token in [
    "GTT 0.1.17", "deterministic", "forward-auto-reverse-forward", "NATIVE_DRIVETRAIN_SCENARIO.json",
    "does **not** claim", "125/130 (96.2%)",
]:
