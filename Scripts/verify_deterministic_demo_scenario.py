...
scenario_step = 'Evaluate spike damage persistence and workshop recovery scenario'
...
checks = {
    ...
    'opt-in commandline': 'GTTDemoSmokeScenario' in cpp and 'GTTDemoSmokeScenario' in recovery_cpp and 'GTTDemoSmokeScenario' in smoke,
    ...
    'evaluator schema v9': 'gtt.demo-scenario.v9' in eval_ps and 'required_step_count=28' in eval_ps,
    'evaluator retains core': all(s in eval_ps for s in core_steps) and 'DEMO_SCENARIO_COMPLETE result=PASS steps=26' in eval_ps,
    'evaluator recovery gates': all(x in eval_ps for x in ['damage_persistence_passed', 'workshop_recovery_passed', 'damage_recovery_complete']),
    'demo gate consumes scenario': "scenario.result -ne 'PASS'" in demo,
    'workflow order': workflow_order_ok,
    'extended packaged runtime': '-MinimumAliveSeconds 90 -LaunchTimeoutSeconds 105' in workflow and '-MinimumRuntimeSeconds 90' in workflow,
    'artifact retained': '\\DEMO_SCENARIO.json' in workflow,
    'version': "default: '0.0.92'" in workflow,
}
failed = [name for name, ok in checks.items() if not ok]
if failed:
    raise SystemExit('0.0.92 deterministic demo scenario verification failed: ' + ', '.join(failed))
print(f'0.0.92 deterministic demo scenario verification passed ({len(checks)} checks).')
