...
def verify_planner_model() -> None:
    ...
    for token in (
        "EGTTLogisticsPriorityLane",
        "FGTTLogisticsRoutePlan",
        "RoadScore",
        "CargoScore",
        "ConsequencePressure",
        "GetCargoConflictHoldCapMinutes",
        "BuildSummary",
    ):
    ...
    for token in (
        ...
        "AssessJobReadiness(FarmCargoJob)",
        "GetRoadPriorityUrgency()",
        "GetCargoPriorityUrgency()",
        "GetCargoReservationCount()",
        "GetCargoBacklogPressure()",
        "IsRoadCourierWindowOpen()",
        "IsCargoDepotWindowOpen()",
        "GetWoodYardDemand()",
        "GetHillFarmDemand()",
    ):
    ...
    for token in (
        "GetFailedRuns() - Logistics->GetCompletedRuns() / 3",
        "GetCargoFailedRuns() - Logistics->GetCargoCompletedRuns() / 3",
        ", 0, 4)",
        "Plan.BacklogPressure * 2",
    ):
...
def verify_conflict_hold_is_real_gameplay() -> None:
    ...
    for token in (
        ...
        "ScoreGap >= 50 ? 0.55f : (ScoreGap >= 30 ? 0.65f : 0.75f)",
        "FMath::Max(10",
    ):
    ...
    for token in (
        ...
        "FGTTLogisticsRoutePlanner::GetCargoConflictHoldCapMinutes(GetWorld())",
        "FMath::Min(PriorityLimitedHold, ConflictHoldMinutes)",
        "ROUTE CONFLICT CAP %d MIN",
