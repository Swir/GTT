...
checks = {
    ...
    'win64 milestone': "default: '0.1.14'" in workflow and 'RUNTIME_SMOKE.json' in workflow and 'DEMO_TECHNICAL_GATE.json' in workflow and 'WIN64_PREFLIGHT.json' in workflow and 'BUILD_ATTEMPT.json' in workflow,
}
failed = [name for name, ok in checks.items() if not ok]
if failed: raise SystemExit('Native roadblock runtime gate verification failed: ' + ', '.join(failed))
print(f'Native roadblock runtime evidence gate retained under current 0.1.14 candidate workflow ({len(checks)}/{len(checks)} checks).')
