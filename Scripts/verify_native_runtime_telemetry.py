...
for token in [
    "UGTTNativeRuntimeTelemetrySubsystem", "UTickableWorldSubsystem", "SampleFieldmaster", "EvidenceSecondsByVehicle",
]:
    assert token in header, f"missing telemetry subsystem header token: {token}"

for token in [
    "NATIVE_FIELDMASTER_RUNTIME_TELEMETRY", "UGTTNativeAxleTractionSubsystem", "SampleSnapshot", "GetWheelState",
    "SlipMagnitude", "SlipAngle", "GetCurrentGear", "GetThrottleInput", "GetBrakeInput", "GetSteeringInput",
    "ValidWheels", "ContactWheels", "bSuspensionRuntimeReady", "GetTowLoadFactor", "IsLegacyTakeoverActive",
    "IsNativeFieldmasterReady",
]:
    assert token in source, f"missing live telemetry token: {token}"

for token in [
    "NATIVE_CHAOS_RUNTIME.json", "gtt.native-chaos-runtime.v1", "ExpectedGitSha", "NATIVE_FIELDMASTER_RUNTIME_TELEMETRY",
    "NATIVE_PHYSICS_EVIDENCE", "NATIVE_WHEEL_SETUP_EVIDENCE", "NATIVE_PHYSICS_FALLBACK", "FIELDMASTER_MOTION",
    "FIELDMASTER_CONTROL", "max_valid_wheels", "max_contacts", "max_suspension_samples", "max_speed_kmh",
    "observed_gears", "command_samples", "result = $result",
]:
    assert token in evaluator, f"missing runtime evaluator contract token: {token}"

for token in [
    "workflow_dispatch:", "runs-on: [self-hosted, windows, x64, unreal-5.8]",
    "run_win64_attested_candidate_acceptance.ps1", "NATIVE_AUTHORITY_RUNTIME.json",
    "Upload sealed Win64 evidence", "Win64-failure-diagnostics",
]:
    assert token in workflow, f"Win64 workflow missing sealed telemetry evidence token: {token}"
for token in ["evaluate_native_chaos_runtime.ps1", "evaluate_native_authority_runtime.ps1"]:
...
for token in ["NATIVE_CHAOS_RUNTIME.json", "NATIVE_AUTHORITY_RUNTIME.json", "Get-FileHash -Algorithm SHA256"]:
    assert token in attestor, f"candidate attestor no longer seals runtime evidence: {token}"

for token in [
    "NATIVE_CHAOS_RUNTIME.json", "gtt.native-chaos-runtime.v1", "native_chaos_runtime='PASS'",
    "max_valid_wheels", "max_contacts", "max_suspension_samples", "max_speed_kmh", "command_samples",
]:
...
for token in ["NATIVE_CHAOS_RUNTIME.json", "gtt.native-chaos-runtime.v1", "four valid", "FIELDMASTER_MOTION", "FIELDMASTER_CONTROL", "visual acceptance"]:
    assert token.lower() in playtest.lower(), f"0.1.15 playtest missing: {token}"
for token in ["UGTTNativeRuntimeTelemetrySubsystem", "evaluate_native_chaos_runtime.ps1", "NATIVE_CHAOS_RUNTIME.json", "schema 5", "does **not** claim"]:
