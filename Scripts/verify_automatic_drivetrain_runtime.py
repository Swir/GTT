...
for token in [
    "UGTTNativeDriveDynamicsSubsystem", "SetThrottleInput(EffectiveThrottle)", "SetSteeringInput(EffectiveSteering)",
    "SetBrakeInput", "automatic forward gears",
]:
    if token not in movement:
        errors.append(f"Fieldmaster movement missing 0.1.16 delegation token: {token}")

for token in ["LastObservedGear", "AutomaticForwardGearChangeCount", "DirectionShiftCommitCount", "GearCommandCount"]:
    if token not in authority_h:
        errors.append(f"drivetrain authority state missing: {token}")

for token in [
    "DirectionShiftReleaseSpeedKmh = 3.5f", "GearMatchesDirection", "NATIVE_AUTOMATIC_GEAR_SHIFT",
    "NATIVE_AUTOMATIC_GEARBOX_EVIDENCE", "NATIVE_DIRECTION_SHIFT_COMMIT", "bDirectionInterlock = true",
    "FinalThrottle = 0.0f", "DirectionInterlockBrakeMin", "Movement->SetTargetGear(Authority.StableDirection, true)",
...
for token in ["bUseAutomaticGears = true", "bUseAutoReverse = false", "ForwardGearRatios", "ChangeUpRPM", "ChangeDownRPM"]:
    if token not in powertrain:
        errors.append(f"Chaos powertrain automatic gearbox contract missing: {token}")

for token in ["signed_speed_kmh", "automatic_gears=%s", "forward_gears=%d", "TransmissionSetup.bUseAutomaticGears", "TransmissionSetup.ForwardGearRatios.Num()"]:
    if token not in telemetry:
        errors.append(f"runtime telemetry missing automatic drivetrain evidence: {token}")

for token in [
    "automatic_gear_samples", "configured_forward_gears", "max_forward_gear_observed", "unsafe_direction_shift_commits",
    "max_direction_shift_commit_speed_kmh", "automatic_gears' 'YES", "forward_gears", "NATIVE_DIRECTION_SHIFT_COMMIT", "3.75",
]:
...
for token in [
    "Automatic upshift freedom", "Forward-to-reverse interlock", "Safe reverse commit", "Rattleback parity",
    "Mulebox parity", "unsafe_direction_shift_commits", "visual acceptance",
]:
    if token.lower() not in playtest.lower():
        errors.append(f"0.1.16 playtest missing: {token}")
for token in ["GTT 0.1.16", "automatic-transmission conflict", "NATIVE_AUTOMATIC_GEARBOX_EVIDENCE", "does **not** claim", "125/130 (96.2%)"]:
