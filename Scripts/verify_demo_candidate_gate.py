...
for token in ("default: '0.0.93'",'runs-on: [self-hosted, windows, x64, unreal-5.8]','evaluate_demo_scenario.ps1','evaluate_demo_candidate.ps1','DEMO_SCENARIO.json','DEMO_TECHNICAL_GATE.json','GTT_RUNTIME.log','actions/upload-artifact@v4','-MinimumAliveSeconds 105 -LaunchTimeoutSeconds 120'):assert token in workflow,f'missing Win64 candidate workflow token: {token}'
assert 'verify_demo_candidate_gate.py' in release_verify;assert '0.0.93' in playtest and '0.0.93' in changelog
...
for token in ('ROADBLOCK_SPIKE_CONSEQUENCE','NATIVE_CHAOS','tire_before','tire_after','tire_delta','gtt.demo-scenario.v10','physical_crossing_passed','wheel_risk_after','POST_SPIKE_ESCAPE','damage_persistence_passed','workshop_recovery_passed','structural_persistence_passed','structural_repair_passed','required_step_count=30'):assert token in scenario,f'missing 0.0.93 structural evidence token: {token}'
print('[OK] Win64 Native structural-persistence candidate gate verified; workflow default 0.0.93; roadmap 125/130.')
