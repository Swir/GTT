...
for token in ("default: '0.0.81'",'runs-on: [self-hosted, windows, x64, unreal-5.8]','evaluate_demo_candidate.ps1','DEMO_TECHNICAL_GATE.json','GTT_RUNTIME.log','actions/upload-artifact@v4'):
 assert token in workflow,f'missing Win64 candidate workflow token: {token}'
assert 'verify_demo_candidate_gate.py' in release_verify; assert '0.0.80' in playtest and '0.0.80' in changelog
...
print('[OK] Win64 demo candidate evidence gate verified; workflow default advanced to 0.0.81; roadmap 125/130.')
