...
for token in ("default: '0.0.89'",'runs-on: [self-hosted, windows, x64, unreal-5.8]','evaluate_demo_scenario.ps1','evaluate_demo_candidate.ps1','DEMO_SCENARIO.json','DEMO_TECHNICAL_GATE.json','GTT_RUNTIME.log','actions/upload-artifact@v4'):assert token in workflow,f'missing Win64 candidate workflow token: {token}'
assert 'verify_demo_candidate_gate.py' in release_verify;assert '0.0.89' in playtest and '0.0.89' in changelog
...
for token in ('ROADBLOCK_SPIKE_CONSEQUENCE','NATIVE_CHAOS','tire_before','tire_after','tire_delta','gtt.demo-scenario.v6'):assert token in ((ROOT/'Scripts/evaluate_demo_scenario.ps1').read_text(encoding='utf-8')),f'missing 0.0.89 Native roadblock evidence token: {token}'
print('[OK] Win64 Native roadblock consequence candidate gate verified; workflow default 0.0.89; roadmap 125/130.')
