...
for token in (
    ...
    "scenario.result -ne 'PASS'",
    "native.result -ne 'PASS'",
    "ExpectedGitSha",
    "Fieldmaster",
    "Rattleback82",
    "Mulebox1200",
    "NATIVE_CHAOS_SMOKE_READY",
    "gtt.native-chaos-runtime.v1",
    "native_chaos_runtime",
    "DEMO_TECHNICAL_GATE.json",
    "DEMO_VISUAL_ACCEPTANCE.json",
    "RequireVisual",
    "scenario.schema -ne 'gtt.demo-scenario.v11'",
    "required_step_count -ne 33",
    "structural_handling_passed",
    "structural_reload_handling_passed",
    "structural_drive_recovery_passed",
    "structural_drive_complete",
    "structural_limp_home",
):
    assert token in gate, f"missing demo evidence gate token: {token}"

# 0.0.94 introduced technical-gate schema 5. Later milestones add evidence
# manifests and increment the schema, so verify the historical floor instead
# of requiring a stale literal.
schema_match = re.search(r"(?m)^\s*schema=(\d+)\s*$", gate)
assert schema_match and int(schema_match.group(1)) >= 5, (
    "demo technical gate schema regressed below 0.0.94 schema-5 floor: "
    f"{schema_match.group(1) if schema_match else 'missing'}"
)
...
assert runtime_match, "Win64 candidate workflow smoke lifetime/timeout is not parseable"
minimum_alive = int(runtime_match.group(1))
launch_timeout = int(runtime_match.group(2))
assert minimum_alive >= 125, f"Win64 candidate runtime regressed below 125 seconds: {minimum_alive}"
assert launch_timeout > minimum_alive, (
    f"Win64 candidate launch timeout must exceed minimum alive time: {launch_timeout}<={minimum_alive}"
)

assert "verify_demo_candidate_gate.py" in release_verify
assert "0.0.94" in playtest and "0.0.94" in changelog

# Preserve the historic release-gate truth while validating the current
# SVG-only roadmap presentation. The retired character meter must not return.
for token in (
    ...
    assert token in roadmap, f"roadmap SVG-only progress mismatch: {token}"
...
assert len(checks) == 130 and done == 125 and open_items == 5, (done, open_items, len(checks))
assert round(done * 100.0 / len(checks), 1) == 96.2
...
for token in (
    "ROADBLOCK_SPIKE_CONSEQUENCE",
    "NATIVE_CHAOS",
    "tire_before",
    "tire_after",
    "tire_delta",
    "gtt.demo-scenario.v11",
    "physical_crossing_passed",
    "wheel_risk_after",
    "POST_SPIKE_ESCAPE",
    "damage_persistence_passed",
    "workshop_recovery_passed",
    "structural_persistence_passed",
    "structural_repair_passed",
    "structural_handling_passed",
    "structural_reload_handling_passed",
    "structural_drive_recovery_passed",
    "structural_drive_complete",
    "required_step_count=33",
):
    assert token in scenario, f"missing 0.0.94 structural limp-home evidence token: {token}"
