...
for token in ("default: '0.0.83'",'runs-on: [self-hosted, windows, x64, unreal-5.8]','evaluate_demo_scenario.ps1','evaluate_demo_candidate.ps1','DEMO_SCENARIO.json','DEMO_TECHNICAL_GATE.json','GTT_RUNTIME.log','actions/upload-artifact@v4'):assert token in workflow,f'missing Win64 candidate workflow token: {token}'
assert 'verify_demo_candidate_gate.py' in release_verify;assert '0.0.83' in playtest and '0.0.83' in changelog
...
print('[OK] Win64 vehicle/crime demo candidate gate verified; workflow default 0.0.83; roadmap 125/130.')
