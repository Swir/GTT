...
checks = {
    "director exposes complete courier stages": all(token in header for token in ("Idle", "CollectParts", "DeliverParts", "TryStartContract", "GetParcelIntegrity", "GetObjectiveText")),
    ...
    "road courier has player-facing title and payout": 'return TEXT("Village Parts Courier")' in contracts and "if (JobTag == RoadRunJob) return 260;" in contracts and "if (JobTag == RoadRunJob) return 390;" in contracts,
    ...
    "pickup and drop markers are compact gameplay guidance": all(token in director for token in ("PARTS DEPOT\\nCOURIER PICKUP", "NORTH WOOD YARD\\nCOURIER DROP", "SetMarkerState(true, false)", "SetMarkerState(false, true)")),
    ...
    "wanted blocks legal handoff while timer runs": "COURIER HANDOFF BLOCKED" in director and "Wanted->GetWantedLevel() > 0" in director and "TimeRemaining = FMath::Max(0.0f, TimeRemaining - DeltaSeconds)" in director,
    ...
    "playtest covers full ROAD loop": all(token in playtest for token in ("Village Parts Courier", "Paid ROAD preparation", "Speed, condition and crash risk", "Police consequence", "Win64/demo evidence")),
    "changelog records milestone and demo honesty": "GTT 0.1.2" in changelog and "Traffic-Risk Economy" in changelog and "No demo Release is authorized" in changelog,
...
print(f"[OK] GTT 0.1.2 Rattleback ROAD courier is wired through fleet prep, HUD, traffic/crash risk, police, economy and save ({len(checks)} checks); roadmap {done}/{total} = {percent:.1f}%.")
