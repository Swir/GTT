...
foreach ($line in $telemetry) {
    ...
    if ($null -ne $gear) { [void]$observedGears.Add($gear) }
...
Write-Host "[GTT] Native Chaos runtime telemetry gate: PASS ($($telemetry.Count) samples, speed=$([Math]::Round($maxSpeed,2)) km/h, contacts=$maxContacts/4, suspension=$maxSuspensionSamples/4, commands=$commandSamples)."
