...
$crossingMatch=[regex]::Match($log,'DEMO_SCENARIO_ROADBLOCK_CROSSING vehicle=(Rattleback82|Mulebox1200) result=PASS roadblock_hits=([0-9]+)')
$physicalCrossing=$crossingMatch.Success;if(-not$physicalCrossing){$errors+='deterministic Native vehicle did not physically cross the active spike strip'}
$handlingMatch=[regex]::Match($log,'DEMO_SCENARIO_HANDLING_CONSEQUENCE vehicle=(Rattleback82|Mulebox1200) result=PASS tire_before=([0-9.]+) tire_after=([0-9.]+) wheel_risk_before=([0-9.]+) wheel_risk_after=([0-9.]+)')
$handlingEvidence=$handlingMatch.Success;$wheelRiskBefore=0.0;$wheelRiskAfter=0.0
if($handlingEvidence){$wheelRiskBefore=[double]$handlingMatch.Groups[4].Value;$wheelRiskAfter=[double]$handlingMatch.Groups[5].Value;if($wheelRiskAfter -le $wheelRiskBefore){$handlingEvidence=$false}}
if(-not$handlingEvidence){$errors+='Native handling consequence did not prove increased wheel risk'}
if($log -notmatch 'DEMO_SCENARIO_COMPLETE result=PASS steps=25'){$errors+='physical roadblock scenario did not complete with 25-step PASS'}
