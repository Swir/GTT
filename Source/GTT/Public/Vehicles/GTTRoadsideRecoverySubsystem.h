...
UENUM(BlueprintType)
enum class EGTTRoadsideRecoveryMode : uint8
{
    None,
    RoadsideAssistance,
    PoliceImpound
...
/**
 * Gameplay recovery layer for player-owned Native road vehicles.
 * A genuinely stranded Rattleback/Mulebox is routed back through the existing
 * economy, wanted, workshop and legacy-mirror save state instead of being reset
 * by a disconnected debug teleport.
 */
UCLASS()
