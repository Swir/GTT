...
void UGTTVillagePresentationSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
    ...
    for (TActorIterator<AGTTPrototypeWorld> It(&InWorld); It; ++It)
    {
        bHasPrototypeWorld = true;
        break;
    }
...
{
    if (bPresentationBuilt || !GetWorld())
    {
        return;
    }
    ...
    // Main village loop: restrained roadside furniture gives the roads a readable rhythm.
    ...
    // Reflector posts around the loop make corners legible from vehicle speed without adding UI arrows.
    for (int32 I = -5; I <= 5; ++I)
    {
    ...
    // Farm/wood-yard dressing: original source-built props only.
    for (int32 Bale = 0; Bale < 8; ++Bale)
    {
        const float X = 5000.0f + (Bale % 4) * 190.0f;
        const float Y = 3350.0f + (Bale / 4) * 220.0f;
    ...
    {
        const float X = 7550.0f + (Log % 5) * 180.0f;
        const float Y = 1120.0f + (Log / 5) * 150.0f;
...
void UGTTVillagePresentationSubsystem::RefreshPresentation()
{
    ...
    for (APointLight* Lamp : StreetLights)
    {
        if (IsValid(Lamp) && Lamp->GetLightComponent())
        {
            Lamp->GetLightComponent()->SetVisibility(bNight);
        }
    }

    // Keep useful signs around the player but remove the giant wall-of-debug-text effect at distance.
    ...
    constexpr float LabelVisibleDistance = 1900.0f;
    const float LabelVisibleDistanceSq = FMath::Square(LabelVisibleDistance);
