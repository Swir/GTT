...
void UGTTNativeStabilitySubsystem::EvaluateFieldmaster(AGTTFieldmasterNativePawn* NativePawn, float DeltaTime)
{
    ...
    const float Risk = ComputeStabilityRisk(NativePawn, Contacts, SpeedKmh, TowLoadFactor, SwayRisk, LoadTransferRisk);
    ...
    const bool bIntervention = SpeedKmh >= ActiveInterventionSpeed && (Risk >= 0.58f || bSustainedLowContact || bSevereAttitude || bSustainedTrailerSway || bSustainedLoadTransfer);
    ...
    if (bIntervention)
    {
        ...
        AppliedBrake = FMath::Clamp(0.18f + Risk * 0.34f + TowLoadFactor * 0.12f + SwayRisk * 0.15f + LoadTransferBrake - TireAssist, 0.16f, 0.82f);
        Movement->SetThrottleInput(0.0f);
        Movement->SetBrakeInput(AppliedBrake);

        if (bSevereAttitude || Contacts <= 1 || (bSustainedTrailerSway && SwayRisk >= 0.72f) || (bSustainedLoadTransfer && FMath::Abs(SideBias) >= 0.62f))
        {
    ...
    UE_LOG(LogGTT, Log,
        TEXT("NATIVE_STABILITY_EVIDENCE vehicle=RustyFieldmaster60 speed_kmh=%.1f contacts=%d/4 clearances_cm=[%.1f,%.1f,%.1f,%.1f] roll=%.1f pitch=%.1f risk=%.2f tow_load=%.2f sway_risk=%.2f load_transfer=%.2f front_rear_bias=%.2f side_bias=%.2f sway_s=%.2f load_transfer_s=%.2f low_contact_s=%.2f intervention=%s brake=%.2f tire_level=%d trailer=%s"),
        SpeedKmh, Contacts, ClearancesCm[0], ClearancesCm[1], ClearancesCm[2], ClearancesCm[3], Rotation.Roll, Rotation.Pitch,
        Risk, TowLoadFactor, SwayRisk, LoadTransferRisk, FrontRearBias, SideBias, State.TrailerSwaySeconds, State.LoadTransferSeconds,
        State.LowContactSeconds, bIntervention ? TEXT("YES") : TEXT("NO"), AppliedBrake, TireLevel, Trailer ? TEXT("ATTACHED") : TEXT("NONE"));
}
...
float UGTTNativeStabilitySubsystem::ComputeStabilityRisk(const AGTTFieldmasterNativePawn* NativePawn, int32 Contacts, float SpeedKmh, float TowLoadFactor, float SwayRisk, float LoadTransferRisk) const
{
    ...
    return FMath::Clamp(FMath::Max3(GroundRisk, HeavyHaulRisk, ChassisRisk), 0.0f, 1.0f);
}