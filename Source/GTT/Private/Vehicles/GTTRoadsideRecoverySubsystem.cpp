...
void UGTTRoadsideRecoverySubsystem::Tick(float DeltaSeconds)
{
    ...
    if (ScanAccumulator < RecoveryScanIntervalSeconds)
    {
        return;
    }
...
bool UGTTRoadsideRecoverySubsystem::IsRecoveryEligible(const AGTTRoadVehicleNativePawn* Vehicle) const
{
    ...
    return SpeedKmh <= MaxRecoverySpeedKmh && (bMechanicalBreakdown || bOutOfFuel || bTiresDisabled || bBodyDisabled);
}
...
void UGTTRoadsideRecoverySubsystem::UpdateVehicle(AGTTRoadVehicleNativePawn* Vehicle, float DeltaSeconds)
{
    ...
    UGTTWantedComponent* Wanted = Driver ? Driver->FindComponentByClass<UGTTWantedComponent>() : nullptr;
    UGTTPlayerEconomyComponent* Economy = Driver ? Driver->FindComponentByClass<UGTTPlayerEconomyComponent>() : nullptr;
    if (!Driver || !Economy)
    {
        return;
    }
    ...
    if (!Runtime.bAnnounced)
    {
        ...
        {
            Economy->PushMessage(TEXT("Vehicle stranded. Roadside recovery and workshop transport inbound."), 5.0f);
            UE_LOG(LogGTT, Log, TEXT("NATIVE_ROADSIDE_RECOVERY_ARMED vehicle=%s"), *Vehicle->GetPersistentVehicleId().ToString());
        }
    }
...
int32 UGTTRoadsideRecoverySubsystem::CalculateRoadsideCost(const AGTTRoadVehicleNativePawn* Vehicle) const
{
    ...
    const int32 DistanceCharge = FMath::Clamp(FMath::RoundToInt(DistanceMeters * 0.035f / 5.0f) * 5, 0, 220);
    const int32 MechanicalCharge = FMath::RoundToInt((1.0f - State.ConditionPercent) * 180.0f + (1.0f - State.TireIntegrity) * 95.0f);
    return FMath::Clamp(140 + DistanceCharge + MechanicalCharge + Vehicle->GetBodyDamageRepairSurcharge(), 140, 700);
}
...
{
    if (Vehicle && Vehicle->GetPersistentVehicleId() == FName(TEXT("Mulebox1200")))
    {
        return WorkshopBaseLocation + FVector(0.0f, 250.0f, 0.0f);
    }
    return WorkshopBaseLocation + FVector(0.0f, -250.0f, 0.0f);
}
...
{
    if (!Vehicle || !Vehicle->GetDriverPawn())
    {
        return;
    }

    APawn* Driver = Vehicle->GetDriverPawn();
    UGTTPlayerEconomyComponent* Economy = Driver->FindComponentByClass<UGTTPlayerEconomyComponent>();
    UGTTWantedComponent* Wanted = Driver->FindComponentByClass<UGTTWantedComponent>();
    if (!Economy)
    {
        return;
    }
    ...
    const int32 Cost = Mode == EGTTRoadsideRecoveryMode::PoliceImpound
        ...
        : CalculateRoadsideCost(Vehicle);

    if (Mode == EGTTRoadsideRecoveryMode::RoadsideAssistance && !Economy->SpendCash(Cost, TEXT("Roadside recovery + workshop service")))
    {
        Economy->PushMessage(FString::Printf(TEXT("Roadside recovery costs $%d. Earn or save enough cash to call it in."), Cost), 6.0f);
        UE_LOG(LogGTT, Warning, TEXT("NATIVE_ROADSIDE_RECOVERY_DENIED vehicle=%s cost=%d reason=INSUFFICIENT_CASH"),
            *Vehicle->GetPersistentVehicleId().ToString(), Cost);
        return;
    }
    ...
    {
        Economy->PushMessage(FString::Printf(TEXT("Roadside recovery complete: $%d. Vehicle delivered and serviced at the workshop."), Cost), 6.0f);
        UE_LOG(LogGTT, Log,
            TEXT("NATIVE_ROADSIDE_RECOVERY_COMPLETE vehicle=%s cost=%d serviced=%s destination=WORKSHOP"),
            *Vehicle->GetPersistentVehicleId().ToString(), Cost, bServiced ? TEXT("YES") : TEXT("NO"));
    }
}
