...
void UGTTFarmCargoBreakdownRecoverySubsystem::EvaluateCargoRecovery()
{
    ...
    if (!BoundPawn)
    {
        ...
        GTT_LOG( Warning, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=WAIT_EXACT_VEHICLE vehicle=%s handoff_locked=YES transfer_allowed=NO"), *BoundId.ToString());
        return;
    }
    ...
    if (bPatchPending)
    {
        if (!bWasPatchPending)
        {
            ...
            GTT_LOG( Display, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=PATCH_CHECKPOINT vehicle=%s saved=%s timer_paused=NO transfer_allowed=NO"), *BoundId.ToString(), bPreRecoveryCheckpointWritten ? TEXT("YES") : TEXT("NO"));
        }
        return;
    }
    ...
    if (bTowPending)
    {
        if (!bWasTowPending)
        {
            ...
            GTT_LOG( Display, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=TOW_CHECKPOINT vehicle=%s saved=%s timer_paused=NO transfer_allowed=NO"), *BoundId.ToString(), bPreRecoveryCheckpointWritten ? TEXT("YES") : TEXT("NO"));
        }
        return;
    }
    ...
    if (WantedLevel >= 2 && Assessment.Severity >= ImpoundCheckpointSeverity)
    {
        if (!bWasImpoundPending)
        {
            ...
            GTT_LOG( Warning, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=IMPOUND_CHECKPOINT vehicle=%s saved=%s transfer_allowed=NO"), *BoundId.ToString(), bPreRecoveryCheckpointWritten ? TEXT("YES") : TEXT("NO"));
        }
        return;
    }
    ...
    if (bWasPatchPending)
    {
        ...
        if (!bIdentityPreserved)
        {
            ...
            GTT_LOG( Error, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=POST_PATCH_VERIFY result=FAIL expected=%s actual=%s handoff_locked=YES transfer_allowed=NO"), *ExpectedId.ToString(), *CargoAuthority->GetBoundCargoVehicleId().ToString());
            return;
        }
        ...
        GTT_LOG( Display, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=POST_PATCH_VERIFY result=PASS vehicle=%s identity_preserved=YES saved=%s timer_paused=NO transfer_allowed=NO"), *ExpectedId.ToString(), bPostSave ? TEXT("YES") : TEXT("NO"));
    ...
    if (bWasTowPending || bWasImpoundPending)
    {
        ...
        if (!bIdentityPreserved)
        {
            ...
            GTT_LOG( Error, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=POST_RECOVERY_VERIFY result=FAIL expected=%s actual=%s handoff_locked=YES"), *ExpectedId.ToString(), *CargoAuthority->GetBoundCargoVehicleId().ToString());
            return;
        }
        ...
        GTT_LOG( Display, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=POST_RECOVERY_VERIFY result=PASS vehicle=%s moved_cm=%.1f identity_preserved=YES saved=%s timer_paused=NO"), *ExpectedId.ToString(), MovedDistanceCm, bPostSave ? TEXT("YES") : TEXT("NO"));
        bPreRecoveryCheckpointWritten = false;
        return;
    }
...
bool UGTTFarmCargoBreakdownRecoverySubsystem::CheckpointPrimarySave(const TCHAR* Reason) const
{
    ...
    if (bSaved)
    {
        GTT_LOG( Display, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=CHECKPOINT reason=%s result=PASS vehicle=%s"), Reason ? Reason : TEXT("unknown"), *RecoveryVehicleId.ToString());
    }
    else
    {
        GTT_LOG( Warning, TEXT("FARM_CARGO_BREAKDOWN_RECOVERY event=CHECKPOINT reason=%s result=FAIL vehicle=%s"), Reason ? Reason : TEXT("unknown"), *RecoveryVehicleId.ToString());
    }
    return bSaved;
}
