...
Current development milestone: **0.1.60 — Trailer stability, road feedback & authored-trailer acceptance bridge**.
...
| 🌾 Legal rural work | Farm cargo, mowing, timber hauling, recovery and heavier trailer/logistics jobs tied to economy and vehicle condition. Farm Cargo locks the actual loaded vehicle to the contract so another vehicle cannot complete its handoff; Heavy Haul scores smooth loaded driving against speed, hitch load, trailer attitude, axle and cargo condition for a one-shot handling bonus, while overspeed/roll/pitch feed the physical hitch-stress envelope. The farm trailer now adds state-driven rear/brake/reverse/hazard feedback, bounded load-aware anti-sway and jackknife-risk handling while the guarded authored-skeletal bridge remains gated behind real UE 5.8 import/runtime acceptance. |
...
Milestone 0.1.57 moves two visible runtime systems forward in one source milestone. ROAD SERVICE keeps a deterministic post-recovery lane-reopening phase instead of disappearing immediately, and the physical farm trailer receives bounded vertical wheel suspension with load-sensitive spring/damping plus a cargo/integrity/stress-sensitive breakable hitch that routes physical failure through the existing authoritative detach path. These changes are covered by dedicated source CI and Unreal playtest requirements; authored skeletal trailer wheel/final hitch assets and real UE 5.8 Win64 packaged/runtime/visual acceptance remain open.
...
Milestone 0.1.60 adds player-visible trailer road feedback and a guarded path from authored source rig to runtime acceptance without pretending the remaining gate is closed. Rear/brake/reverse/hazard lights now reflect real trailer state, loaded anti-sway is bounded by speed/load/condition, and jackknife risk feeds the same physical handling envelope. The authored skeletal bridge validates required bones, final hitch/wheel sockets and PhysicsAsset before activation; Interchange import automation and packaged-runtime evidence hooks can exercise those contracts on a qualifying UE 5.8 Windows runner. Until that exact candidate actually imports, packages, runs and passes visual acceptance, roadmap truth remains 125/130 and demo readiness remains NOT READY.
