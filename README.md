...
Current development milestone: **0.1.46 — packaged workshop repair queue evidence and release-gate integration**.
...
A successful future packaged Farm Cargo candidate must produce `FARM_CARGO_RUNTIME.json` (`gtt.farm-cargo-runtime.v1`), `FARM_CARGO_RECOVERY_RUNTIME.json` (`gtt.farm-cargo-recovery-runtime.v1`), `FARM_CARGO_BREAKDOWN_RUNTIME.json` (`gtt.farm-cargo-breakdown-runtime.v2`), `FARM_CARGO_DISPATCH_RUNTIME.json` (`gtt.farm-cargo-dispatch-runtime.v1`), `FARM_CARGO_DISPATCH_PERSISTENCE_RUNTIME.json` (`gtt.farm-cargo-dispatch-persistence-runtime.v1`), `FARM_CARGO_WORKSHOP_RECOVERY_RUNTIME.json` (`gtt.farm-cargo-workshop-recovery-runtime.v1`), `WORKSHOP_HOURS_RUNTIME.json` (`gtt.workshop-hours-runtime.v1`) and `WORKSHOP_QUEUE_RUNTIME.json` (`gtt.workshop-queue-runtime.v1`). These progressively prove exact-vehicle contract continuity, mid-route save/load + recreated-actor rebinding, emergency patch + exact-ID/body/timer continuity followed by deliberate re-breakdown/paid tow, locked-quote/live-ETA/cancellation/re-request dispatch authority, dispatch-sidecar SaveGame restore/replay safety, tow → hard hold → blocked recall → paid workshop recovery continuity, clock-boundary/closed-service rejection plus exact after-hours emergency quote/service behavior, and finally persisted deferred workshop booking with exact-ID/locked-quote disk checkpointing, substitute rejection and single-debit completion. **None of these manifests is claimed until the packaged Unreal executable actually runs and emits the required PASS evidence.**
