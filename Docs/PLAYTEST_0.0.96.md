...
5. Repeat with an immobilized vehicle. Press `T` or gamepad D-Pad Up. Confirm the quoted tow is explicitly requested and paid, then confirm the vehicle is delivered near the workshop with its damage still present.
...
The `Win64 recovery-choice evidence` workflow packages version 0.0.96 on a real self-hosted UE 5.8 runner and keeps the executable alive long enough for the earlier wanted/spike/save-load/structural route plus the new recovery sequence. Runtime must emit all of:
